...
- **Draft Listing Deletion Logic** (12/10/2024): Corrected a bug that incorrectly prevented the deletion of listings in "Draft" status. The faulty server-side validation in the `deleteListingAction` was removed, allowing drafts to be deleted as expected.
- **Robust Unit Duplication** (11/10/2024): The "Duplicate" unit feature has been completely overhauled. It now uses a server action to create a deep copy of the selected unit in the database, including all its photos, pricing, and policies. The new unit is created with a blank `Unit Ref` and `Unit Name` for the user to fill in, and the user is redirected to the new unit's edit page, ensuring a seamless workflow.
- **Sortable Admin Tables** (11/10/2024): The "Configure Units" table is now fully sortable by `Unit Ref`, `Name`, `Type`, `Guests`, `Price`, and `Status`, with `Unit Ref` as the default sort order. This significantly improves data navigation for administrators.
- **Live "Lowest Price" Display** (11/10/2024): The "Manage Listings" table and the public-facing accommodation detail page now correctly fetch and display the lowest available price from a property's individual units. This ensures pricing is always live and accurate, reflecting the most up-to-date unit information.
- **Image Gallery Sorting & Save Logic** (11/10/2024): Fixed a critical bug in both the main "Property Photo Gallery" and the individual "Unit Photo Gallery" where reordering images would not activate the "Save Changes" button. The change detection logic has been corrected to properly recognize the new image order.
- **Photo Gallery Modal Controls** (11/10/2024): The full-screen photo gallery viewer has been fixed. The close button (`X`) is now functional, and previous/next arrow buttons have been added, allowing users to navigate through the images within the modal.
- **Amenity Label Display** (11/10/2024): Corrected an issue on the public accommodation detail page where internal system tags (e.g., `wifi`) were being displayed instead of the proper, user-friendly labels (e.g., "WiFi"). The font size was also adjusted for better consistency.
- **Next.js Runtime Errors** (11/10/2024): Resolved several client-side runtime errors reported by Next.js, including a `Map is not a constructor` error caused by a naming conflict and a `useFormContext` error on the "Accessibility Features" page, improving overall application stability.
...
- **Unit Setup Enhancements**:
  ...
  - **Layout Details**: Add fields for Floor Level/Building Wing, and Sleeping Arrangement Notes to provide more granular detail about the unit's physical layout.
  - **Room Size Measurement**: Add a user preference to select between Square Meters (m²) and Square Feet (ft²), with `m²` as the default. This setting will apply to the `Area` field in the Unit's "Basic Info" and will handle conversions where necessary.
