...
- **Incorrect Date Display on Search Results**: The date range displayed on the `/results` page header is consistently one day earlier than the dates selected in the search form. This appears to be a timezone conversion issue when parsing the date strings from the URL parameters. An initial attempt to fix this by treating the dates as UTC did not resolve the problem.
