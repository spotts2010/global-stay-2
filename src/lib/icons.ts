// Filepath: src/lib/icons.ts
// Centralized React Icons for the project
import { FaPaypal, FaGoogle, FaApple, FaFacebook, FaDatabase } from 'react-icons/fa';
import { TbLayoutSidebarLeftCollapse, TbLayoutSidebarRightCollapse } from 'react-icons/tb';

// Material Design Icons
export { MdKeyboardArrowDown as ChevronDown } from 'react-icons/md';
export { MdKeyboardArrowRight as ChevronRight } from 'react-icons/md';
export { MdKeyboardArrowLeft as ChevronLeft } from 'react-icons/md';
export { MdOutlineKeyboardArrowUp as ChevronUp } from 'react-icons/md';
export { MdMoreHoriz as MoreHorizontal } from 'react-icons/md';
export { MdCheck as Check } from 'react-icons/md';
export { MdClose as X } from 'react-icons/md';
export { MdRadioButtonUnchecked as Circle } from 'react-icons/md';
export { MdFavorite as Heart } from 'react-icons/md';
export { MdLocationOn as MapPin } from 'react-icons/md';
export { MdStar as Star } from 'react-icons/md';
export { MdWifi as Wifi } from 'react-icons/md';
export { MdDirectionsCar as Car } from 'react-icons/md';
export { MdRestaurant as Utensils } from 'react-icons/md';
export { MdEmojiEvents as Award } from 'react-icons/md';
export { MdFitnessCenter as Dumbbell } from 'react-icons/md';
export { MdHotel as Hotel } from 'react-icons/md';
export { MdPerson as User } from 'react-icons/md';
export { MdSettings as Settings } from 'react-icons/md';
export { MdGridView as LayoutGrid } from 'react-icons/md';
export { MdLogout as LogOut } from 'react-icons/md';
export { MdWork as Briefcase } from 'react-icons/md';
export { MdArrowBack as ArrowLeft } from 'react-icons/md';
export { MdMarkEmailUnread as MailWarning } from 'react-icons/md';
export { MdConfirmationNumber as Ticket } from 'react-icons/md';
export { MdViewSidebar as PanelLeft } from 'react-icons/md';
export { MdInventory2 as Package2 } from 'react-icons/md';
export { MdGroup as Users } from 'react-icons/md';
export { MdCalendarToday as Calendar } from 'react-icons/md';
export { MdRemoveRedEye as Eye } from 'react-icons/md';
export { MdAssignment as ClipboardList } from 'react-icons/md';
export { MdBadge as IdCard } from 'react-icons/md';
export { MdMenuBook as BookOpenText } from 'react-icons/md';
export { MdUpload as Upload } from 'react-icons/md';
export { MdLink as LinkIcon } from 'react-icons/md';
export { MdFlag as Flag } from 'react-icons/md';
export { MdDelete as Trash2 } from 'react-icons/md';
export { MdCheckCircle as CheckCircle } from 'react-icons/md';
export { MdChecklistRtl as ListChecks } from 'react-icons/md';
export { MdNotifications as Bell } from 'react-icons/md';
export { MdAddCircle as PlusCircle } from 'react-icons/md';
export { MdEdit as FilePen } from 'react-icons/md';
export { MdCardGiftcard as Gift } from 'react-icons/md';
export { MdWarningAmber as TriangleAlert } from 'react-icons/md';
export { MdPersonAdd as UserPlus } from 'react-icons/md';
export { MdEventAvailable as CalendarCheck } from 'react-icons/md';
export { MdMail as Mail } from 'react-icons/md';
export { MdMailOutline as MailOpen } from 'react-icons/md';
export { MdLanguage as Languages } from 'react-icons/md';
export { MdFlight as Plane } from 'react-icons/md';
export { MdAutoFixHigh as Wand } from 'react-icons/md';
export { MdPhone as Phone } from 'react-icons/md';
export { MdCheckCircleOutline as CheckCircle2 } from 'react-icons/md';
export { MdContentCopy as Copy } from 'react-icons/md';
export { MdCameraAlt as Camera } from 'react-icons/md';
export { MdPublic as Globe } from 'react-icons/md';
export { MdAccountBalanceWallet as Wallet } from 'react-icons/md';
export { MdFlag as Milestone } from 'react-icons/md';
export { MdChatBubbleOutline as MessageSquareWarning } from 'react-icons/md';
export { MdLaptop as Laptop } from 'react-icons/md';
export { MdLock as Lock } from 'react-icons/md';
export { MdSmartphone as Smartphone } from 'react-icons/md';
export { MdTablet as Tablet } from 'react-icons/md';
export { MdBed as Bed } from 'react-icons/md';
export { MdImage as ImageIcon } from 'react-icons/md';
export { MdCloudUpload as UploadCloud } from 'react-icons/md';
export { MdSave as Save } from 'react-icons/md';
export { MdDragIndicator as GripVertical } from 'react-icons/md';
export { MdHelpOutline as ShieldQuestion } from 'react-icons/md';
export { MdAttachMoney as DollarSign } from 'react-icons/md';
export { MdBed as BedDouble } from 'react-icons/md';
export { MdEventNote as CalendarClock } from 'react-icons/md';
export { MdAccessible as Accessibility } from 'react-icons/md';
export { MdApartment as Building } from 'react-icons/md';
export { MdInventory2 as Box } from 'react-icons/md';
export { MdInfoOutline as Info } from 'react-icons/md';
export { MdEditSquare as SquarePen } from 'react-icons/md';
export { MdArchive as Archive } from 'react-icons/md';
export { MdRotateLeft as RotateCcw } from 'react-icons/md';
export { MdFilterList as ListFilter } from 'react-icons/md';
export { MdAutoAwesome as Sparkles } from 'react-icons/md';
export { MdSmartToy as Bot } from 'react-icons/md';
export { MdPool as Waves } from 'react-icons/md';
export { MdGridView as Grid3x3, MdCalendarViewMonth as CalendarIcon } from 'react-icons/md';
export { MdChevronRight as PanelRight } from 'react-icons/md';
export { MdWatchLater as Hourglass } from 'react-icons/md';
export { MdPersonRemove as UserX } from 'react-icons/md';
export { MdHowToReg as UserCheck } from 'react-icons/md';
export { MdBlock as Ban } from 'react-icons/md';
export { MdPayment as Banknote } from 'react-icons/md';
export { MdOutlineHome as Home } from 'react-icons/md';
export { MdSupport as LifeBuoy } from 'react-icons/md';
export { MdArticle as FileText } from 'react-icons/md';
export { MdHelpOutline as HelpCircle } from 'react-icons/md';
export { MdOutlineLock as GlobeLock } from 'react-icons/md';
export { MdTune as SlidersHorizontal } from 'react-icons/md';
export { MdOutlineCalendarMonth as CalendarDays } from 'react-icons/md';
export { MdBathtub as Bath } from 'react-icons/md';
export { MdArrowUpward as ArrowUp } from 'react-icons/md';
export { MdArrowDownward as ArrowDown } from 'react-icons/md';
export { MdUnfoldMore as ChevronsUpDown } from 'react-icons/md';
export { MdOutlineViewList as LayoutList } from 'react-icons/md';
export { MdOutlineSettings as Cog } from 'react-icons/md';
export { MdSearch as Search } from 'react-icons/md';
export { MdShield as Shield } from 'react-icons/md';
export { MdCreditCard as CreditCard } from 'react-icons/md';
export { MdCommit as GitCommit } from 'react-icons/md';
export { MdFormatBold as Bold } from 'react-icons/md';
export { MdFormatItalic as Italic } from 'react-icons/md';
export { MdFormatUnderlined as Underline } from 'react-icons/md';
export { MdFormatStrikethrough as Strikethrough } from 'react-icons/md';
export { MdFormatListBulleted as List } from 'react-icons/md';
export { MdFormatListNumbered as ListOrdered } from 'react-icons/md';
export { MdUndo as Undo } from 'react-icons/md';
export { MdRedo as Redo } from 'react-icons/md';
export { MdTitle as Heading1 } from 'react-icons/md';
export { MdTitle as Heading2 } from 'react-icons/md';
export { MdTitle as Heading3 } from 'react-icons/md';

// Ant Design Icons (for loaders)
export { AiOutlineLoading3Quarters as Loader2 } from 'react-icons/ai';

export {
  FaPaypal,
  FaGoogle,
  FaApple,
  FaFacebook,
  FaDatabase as Database,
  TbLayoutSidebarLeftCollapse,
  TbLayoutSidebarRightCollapse,
};
export { MdForum as Forum } from 'react-icons/md';
